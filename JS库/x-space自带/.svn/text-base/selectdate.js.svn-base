...
//	var pElement=(!pElt ? document.body:getobject(pElt));
	...
	var div=document.getElementById("divHide");
  	if(!div) {
  		...
  		div.id = "divHide";
  		div.style.cssText = "left:0;top:0;overflow:hidden;position:absolute;visibility:hidden;border:1px solid #FFBE00;";
...
function showPicker() {
	...
	for (var intWeek = 0; intWeek < 6; intWeek++) {
		...
		if(intWeek+1==6 && tformat > timeswitch)
			html+="<td colspan='4' borderColor=#ffffff><div style='margin:-1.5px;'><input maxlength='2' onmousedown='this.select()' name='hour' id='hour' style='border: 1px solid #CCCCCC;font-size:12px;font-family:宋体;line-height:12px;height:12px;width:20px;text-align:center;' size='1' value='"+ (hour<10?'0'+hour:hour) +"' type='text'>时<input maxlength='2' onmousedown='this.select()' value='"+ (minutes<10?'0'+minutes:minutes) +"' style='border: 1px solid #CCCCCC;font-size:12px;font-family:宋体;line-height:12px;height:12px;text-align:center;width:20px' name='minutes' size='1' type='text'>分<input maxlength='2' onmousedown='this.select()' name='second' style='border: 1px solid #CCCCCC;font-size:12px;font-family:宋体;line-height:12px;height:12px;text-align:center;width:20px' value='"+ (second<10?'0'+second:second) +"' size='1' type='text'>秒<div></td>";
		html+="</tr>";
	}
	html+="</table>";
