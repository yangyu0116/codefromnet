 ...
 */
require_once $_SERVER['DOCUMENT_ROOT'].'/config.ini.php';//增加
require('config.php') ;
require_once $INC.'/FCKeditor/custom.php';//增加
require('util.php') ;
...
//2007-1-19增加
function SendResults2( $sFileUrl_original )
{
	echo '<script type="text/javascript">' ;
	echo 'window.parent.setOriginalImg ( "'.$sFileUrl_original.'") ;' ;
	echo '</script>' ;
}
...
while ( true )
{
	...
	$Files->mkdirAll($sServerDir);//2006-8-6增加
	$Files->mkdirAll($sServerDir.'/small');//2006-8-6增加
	$sFilePath = $sServerDir . '/'. $DistFileName ;//2006-8-6增加
	$Width = '500';//2006-8-6增加
	$Height = '';//2006-8-6增加
	$DistFileName_small = $Files->getSmallPicName($DistFileName, $Width, $Height);//2006-8-6增加
	$sFilePath_small = $sServerDir . '/small/'. $DistFileName_small ;//2006-8-6增加
	...
	else
	{
		...
		$Image->Resize($sFilePath, $sFilePath_small, $Width, $Height);//2006-8-6增加
		...
		*/
		$sFileUrl = $Config['UserFilesUrl'].'/small/'.$DistFileName_small;//2006-8-6增加

		break ;
	}
}

$sFileUrl_original = $Config['UserFilesUrl'].'/'.$DistFileName;//2007-1-19增加
SendResults2($sFileUrl_original) ;//2007-1-19增加
