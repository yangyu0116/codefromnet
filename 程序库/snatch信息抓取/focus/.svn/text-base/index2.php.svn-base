...
class Snatch
{
	...
	function getData($url)
	{
		...
		preg_match_all("/<th>售价<\/th>\r\n	<td style=\"color:red\">(.*?)<\/td>/is", $contents, $Price);
		preg_match_all("/(.*?)万元/is", trim ($Price[1][0]), $Price);
		$Price = $Price[1][0];
		//echo $Price."\r\n";
		...
		preg_match_all("/<th>楼层与朝向<\/th>\r\n	<td>\r\n(.*?)<\/td>/is", $contents, $Louceng);
		//echo trim ($Louceng[1][0])."\r\n";
		preg_match_all("/第(.*?)层/is", trim ($Louceng[1][0]), $Floor);
		preg_match_all("/楼高(.*?)层/is", trim ($Louceng[1][0]), $FloorTotal);
		$pos1 = strpos(trim ($Louceng[1][0]), '[朝向]');
		
		if($pos1)
		{
			$Direction = substr(trim ($Louceng[1][0]), $pos1+13, strlen(trim ($Louceng[1][0]))-13-$pos1);
		}
		//$Direction = $this->getValue(trim ($Louceng[1][0]), '[朝向]', '');
		//preg_match_all("[朝向](.*?)/is", trim ($Louceng[1][0]), $Direction);
		...
		preg_match_all("/<th>物业收费<\/th>\r\n	(.*?)<\/td>/is", $contents, $RealtyFee);
		//print_r ($RealtyFee);
		preg_match_all("/<td>(.*?)元/is", trim ($RealtyFee[1][0]), $RealtyFee);
		$RealtyFee = trim($RealtyFee[1][0]);
		//echo $RealtyFee."\r\n";
		...
		$sql = "INSERT INTO `house` (`Pid`,`Title`, `AreaID`,`CityID`,`DistID`,`CommID`,`Fitment`,`Area`,`PriceType`,`Price`,`Room`,`Hall`,`Loo`,`Floor`,`FloorTotal`,`Direction`,`RealtyFee`,`Facility`,`Content`,`PostTime`,`Contact`,`Email`,`Tel`,`Mobile`,`ValidPeriod`,`IfPersonal`) ";
		$sql .= "VALUES ('2','".$Title."','1', '1', '".$DistID."','".$CommID."','".$Fitment."','".$Area."','1','".$Price."','".$Room."','".$Hall."','".$Loo."','".$Floor."','".$FloorTotal."','".$Direction."','".$RealtyFee."','".$Facility."','".$Content."','".$PostTime."','".$Contact."','".$Email."','".$Tel."', '".$Mobile."', '3', '1') ";
		//echo $sql."\r\n";
...
$url = 'http://esf.focus.cn/newesf/sale_frame.php';//（个人出售房源）必须设置
$start = 1;//起始页，必须设置
$end = 3;//结束页，必须设置
$MaxID = 10328038;//最大的ID，必须设置
$MinID = 10325165;//最小的ID，必须设置
//10211616-10206601,10137295-10133591,884053-871507,871486-855821,854910-851856,851840-848375,848362-846575,846556-840634
...
//$Snatch->getData('http://esf.focus.cn/esf/sale/843183.html');
